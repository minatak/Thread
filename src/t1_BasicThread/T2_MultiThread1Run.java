...
public class T2_MultiThread1Run {
	public static void main(String[] args) {
		System.out.println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
		System.out.println("main(1).현재 클래스명 : T2_MultiThread1");
		System.out.println("main(1).현재 실행중인 스레드 이름 : " + Thread.currentThread().getName());
		System.out.println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
		System.out.println();
		
		T2_MultiThread1 t1 = new T2_MultiThread1(); // 스레드를 상속받은 객체. 그래서 이것은 스레드이다.
		// t1.run(); // 스레드를 실행할때는 run() 메소드가 아닌 start() 메소드로 실행한다. run은 그냥 오버라이드 하는 메소드에 불과함.
		t1.start();
